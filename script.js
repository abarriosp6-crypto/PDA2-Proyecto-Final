...
// Muestra derivación
// Muestra derivación en formato compacto
function displayDerivation(input, derivationType) {
    ...
    // Paso 1: S → A Zs
    const prev1 = current;
    current = current.replace('S', 'A Zs');
    html += `
    <div class="derivation-step">
        <div class="step-content">
            <span class="from-expr">${prev1}</span>
            <span class="arrow">⇒</span>
            <span class="to-expr">${current}</span>
        </div>
    </div>`;
    ...
    let aExpansion = 'A';
    for (let i = 0; i < n; i++) {
        const prevA = aExpansion;
        aExpansion = aExpansion.replace('A', 'x x A y');
        const prevTotal = current.replace('A', prevA);
        const newTotal = current.replace('A', aExpansion);
        
        html += `
        <div class="derivation-step">
            <div class="step-content">
                <span class="from-expr">${prevTotal}</span>
                <span class="arrow">⇒</span>
                <span class="to-expr">${newTotal}</span>
            </div>
        </div>`;
    ...
    <div class="derivation-step">
        <div class="step-content">
            <span class="from-expr">${prevB}</span>
            <span class="arrow">⇒</span>
            <span class="to-expr">${current}</span>
        </div>
    </div>`;
    ...
    let bExpansion = 'B';
    for (let i = 0; i < m - 1; i++) {
        const prevB = bExpansion;
        bExpansion = bExpansion.replace('B', 'y B x x x');
        const prevTotal = current.replace('B', prevB);
        const newTotal = current.replace('B', bExpansion);
    ...
    // B → yxxx
    const prevFinalB = current;
    current = current.replace('B', `y${'x'.repeat(3)}`);
    html += `
    <div class="derivation-step">
        <div class="step-content">
            <span class="from-expr">${prevFinalB}</span>
            <span class="arrow">⇒</span>
            <span class="to-expr">${current}</span>
        </div>
    </div>`;
    ...
    let zsExpansion = 'Zs';
    for (let i = 0; i < z_len - 1; i++) {
        const prevZs = zsExpansion;
        zsExpansion = zsExpansion.replace('Zs', 'z Zs');
        const prevTotal = current.replace('Zs', prevZs);
        const newTotal = current.replace('Zs', zsExpansion);
    ...
    // Zs → z
    const prevFinalZs = current;
    current = current.replace('Zs', 'z');
    html += `
    <div class="derivation-step">
        <div class="step-content">
            <span class="from-expr">${prevFinalZs}</span>
            <span class="arrow">⇒</span>
            <span class="to-expr">${current}</span>
        </div>
    </div>`;
